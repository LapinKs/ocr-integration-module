...
class FinetunedLatexOCRClient:
    ...
    # def _normalize_latex(self, latex: str) -> str:
    #     """Улучшенная нормализация LaTeX с исправлением частых ошибок"""
    #     if not isinstance(latex, str):
    #         return ""
    ...
    #     # Исправление специфичных проблем
    #     latex = re.sub(r'\\hat\{\\hat\{', r'\\hat{', latex)

    #     # Удаление пробельных команд
    #     for cmd in [r"\\;", r"\\,", r"\\quad", r"\\qquad", r"\\!", r"\\:"]:
    #         latex = re.sub(cmd, "", latex)

    #     # Нормализация пробелов вокруг операторов
    #     latex = re.sub(r"\s*([\{\}\[\]\(\)=+\-/,])\s*", r"\1", latex)

    #     # Исправление частых опечаток OCR
    #     fixes = [(r'\\fract', r'\\frac'), (r'\\srt', r'\\sqrt'),
    #             (r'\\intt', r'\\int'), (r'\\summ', r'\\sum')]
    #     for pattern, replacement in fixes:
    #         latex = re.sub(pattern, replacement, latex)

    #     # Убираем лишние фигурные скобки в степенях
